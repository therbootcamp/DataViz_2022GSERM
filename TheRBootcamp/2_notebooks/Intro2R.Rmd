...
1. Wouldn't it be great if one did have to select each quarter one by one using filter? The `group_by` and`summarize` idiom saves you from exactly that. Complete the code below to calculate the average Gini coefficients for every `quarter`.  
